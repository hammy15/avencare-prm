    ...
    <span className={cn('inline-flex items-center gap-1.5', className)}>
      ...
      <span
        className={cn('text-xs font-medium', {
          ...
          'text-status-pending': status === 'pending',
          'text-status-flagged': status === 'flagged',
          'text-status-unknown': status === 'unknown',
        })}
      >
        {labels[status]}
      </span>
...
export function StateBadge({
  ...
  const stateClass = ['WA', 'OR', 'ID', 'MT', 'AZ', 'CA', 'AK'].includes(state)
    ? `state-badge-${state}`
    : '';
  return (
    <span className={cn('state-badge', stateClass, className)}>{state}</span>
  );
}
...
export function CredentialBadge({
  ...
  const credentialClass = ['RN', 'LPN', 'CNA', 'APRN'].includes(typeUpper)
