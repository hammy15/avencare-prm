...
export class OhioScraper extends BaseScraper {
  ...
  async verify(params: LicenseLookupParams): Promise<ScraperResult> {
    ...
    try {
      console.log(`[OH Scraper] Starting verification for license: ${params.licenseNumber}`);
      ...
      const submitBtn = await page.$('input[type="submit"], button[type="submit"], button.btn');
      if (submitBtn) {
        await Promise.all([
