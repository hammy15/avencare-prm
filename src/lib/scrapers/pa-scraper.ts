...
/**
 * Pennsylvania State Board of Nursing License Scraper
 * URL: https://www.pals.pa.gov/
 */
export class PennsylvaniaScraper extends BaseScraper {
  config: ScraperConfig = {
    state: 'PA',
    credentialTypes: ['RN', 'LPN', 'CRNP', 'APRN'],
    lookupUrl: 'https://www.pals.pa.gov/',
    timeout: 45000,
  };
  ...
  async verify(params: LicenseLookupParams): Promise<ScraperResult> {
    ...
    try {
      console.log(`[PA Scraper] Starting verification for license: ${params.licenseNumber}`);
  ...
  private async parseResults(page: import('puppeteer').Page, licenseNumber: string): Promise<ScraperResult> {
    ...
    try {
      ...
      if (!status) {
        if (/\bactive\b/i.test(allText) || /\bcurrent\b/i.test(allText)) status = 'Active';
      ...
      return {
        ...
        licenseNumber,
        licenseName: rawData['Name'],
        status: this.normalizeStatus(status || null),
        expirationDate: this.parseDate(rawData['Expiration'] || null),
        unencumbered: !/discipline|action/i.test(allText),
        rawData,
      };
    } catch {
