...
/**
 * Georgia Board of Nursing License Scraper
 * URL: https://verify.llr.sc.gov/
 */
export class GeorgiaScraper extends BaseScraper {
  config: ScraperConfig = {
    state: 'GA',
    credentialTypes: ['RN', 'LPN', 'APRN', 'CNA'],
    lookupUrl: 'https://sos.ga.gov/PLB/verification/',
    timeout: 45000,
  };
  ...
  async verify(params: LicenseLookupParams): Promise<ScraperResult> {
    ...
    try {
      console.log(`[GA Scraper] Starting verification for license: ${params.licenseNumber}`);
